...
router.get("/getOne/:cedula", async (req,res) => {
    try {
        ...
        const data = await patient.find({cedula:`${cedula}`}).limit(1)
        res.json(data)
    }
    catch (error) {
        res.status(500).json({ message: error.message })
...
//----------BUSCAR CITA POR ESPECIALIDAD
router.get("/cita/:cedula", async (req,res) => {
    try {
        const cedula = req.params.cedula;
        const data = await cita.find({cedula:`${cedula}`})
        res.json(data)
    }
    catch (error) {
        res.status(500).json({ message: error.message })
